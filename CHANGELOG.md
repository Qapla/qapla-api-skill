...
- **`versioning.md` now distinguishes the published spec snapshot from the live
  instance.** The snapshot on api.qapla.dev is a static file regenerated by hand,
  so it is the thing that lags; the running service's `info.version` and
  `GET /v2/version` read the same source and always agree. Also noted that the
  deployed value carries a build stamp (`v2.21.10-202609031153`), so parse it as a
  prefix rather than bare semver.
