...
| **Rate limit** | Token bucket (default **300** capacity, refill **150/min** since v2.20.0 — was 120 + 2/sec), Redis-backed. Over limit → `429` with `X-RateLimit-*` headers. Your channel's actual limits come back in the token response (`rate_limit`) |
