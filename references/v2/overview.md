...
| **Time** | CEST, `YYYY-MM-DD HH:MM:SS` (space) | **Mixed — see below.** `parcels`/`orders` are ATOM UTC (`…+00:00`); shipment tracking and `sandbox` are still `YYYY-MM-DD HH:MM:SS` in `Europe/Rome` |
...
| **Time** | ⚠️ **Not uniformly UTC.** `parcels`/`orders` timestamps are ATOM UTC (`…+00:00`). **Shipment tracking** (`statusDate`, `statusUpdatedAt`, `history[].date`) and **sandbox** timestamps are `"Y-m-d H:i:s"` with **no offset**, in `Europe/Rome`. The `updatedAfter` filter is read in that same zone, so polling in UTC **silently loses shipments**. Moving to real UTC is a planned breaking change |
| **Rate limit** | Token bucket **per channel**, Redis-backed. Platform defaults **300** capacity, refill **150/min** (since v2.20.0); a channel can be given its own, and the real ones come back in the token response (`rate_limit`). The bucket refills **one whole cycle at a time** — the full per-minute allowance arrives at once, it does not trickle — so the wait after a `429` is up to a minute. Over limit → `429` carrying **`Retry-After`** (seconds: honour it, do not guess), `X-RateLimit-Limit`, `X-RateLimit-Remaining` and `X-RateLimit-Reset`. ⚠️ **10 x `429` in 5 minutes suspend the key for an hour**, answered `403` and not `429` — so never hammer through a `429` |
...
2. [`endpoints.md`](endpoints.md) — the v2 catalog: what this skill covers in depth, and what is published but not detailed here.
