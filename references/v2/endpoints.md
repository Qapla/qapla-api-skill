...
> the deployed `qore/api` (v2.9.4). The **in-flight** resources exist in the
...
| `shipments` | `POST`/`GET` `/v2/shipments`, `GET /v2/shipments/{id}` | Bulk create (max 100), search, read |
| `labels` | `POST /v2/labels` | Create shipping label (idempotent) |
| `couriers` | `GET /v2/couriers`, `GET /v2/couriers/{code}` | List couriers / get by code |
