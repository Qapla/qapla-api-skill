...
2.14.0 until it was regenerated to 2.21.9 on 2026-09-03 — so check its
`info.version` before trusting it, and be clear about *which* `info.version` you
are reading:

- The **published snapshot** (`v2/swagger/openapi.json` on api.qapla.dev) is a
  static file, regenerated by hand. It is the one that lags, and it can lag behind
  the live API by a release or more.
- The **live instance** reports the truth. `GET /v2/version` and the running
  service's own `info.version` read the same source, so they always agree. The
  deployed value carries a build stamp — e.g. `v2.21.10-202609031153` — so parse
  it as a prefix, not as bare semver.

At the time of writing the two disagree: the published snapshot says `2.21.9`
while <https://api.qapla.it/v2/version> reports `v2.21.10-…`. When they differ,
the live endpoint wins. Other resources (orders, shipments, labels, couriers) are now
